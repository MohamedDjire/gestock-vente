...
// API pour la gestion de la trésorerie (compta_tresorerie)
...
switch ($action) {
    case 'all':
        $sql = "SELECT * FROM stock_compta_tresorerie WHERE id_entreprise = ? ORDER BY date_operation DESC";
    ...
    case 'get':
        ...
        $sql = "SELECT * FROM stock_compta_tresorerie WHERE id_tresorerie = ? AND id_entreprise = ?";
        $stmt = $pdo->prepare($sql);
    ...
    case 'create':
        ...
        $sql = "INSERT INTO stock_compta_tresorerie (date_operation, type_operation, montant, solde, commentaire, id_entreprise) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $pdo->prepare($sql);
        $ok = $stmt->execute([
    ...
    case 'update':
        ...
        if (!$id) response(false, null, 'ID manquant');
        $data = json_decode(file_get_contents('php://input'), true) ?? $_POST;
        $sql = "UPDATE stock_compta_tresorerie SET date_operation=?, type_operation=?, montant=?, solde=?, commentaire=? WHERE id_tresorerie=? AND id_entreprise=?";
        $stmt = $pdo->prepare($sql);
        $ok = $stmt->execute([
            $data['date_operation'] ?? date('Y-m-d'),
            $data['type_operation'] ?? '',
    ...
    case 'delete':
        $id = $_GET['id'] ?? $_POST['id'] ?? null;
        if (!$id) response(false, null, 'ID manquant');
        $sql = "DELETE FROM stock_compta_tresorerie WHERE id_tresorerie = ? AND id_entreprise = ?";
        $stmt = $pdo->prepare($sql);
